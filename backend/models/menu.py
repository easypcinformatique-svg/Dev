...
class Categorie(Base, TimestampMixin):
    ...
    tva_taux: Mapped[float] = mapped_column(Float, default=10.0)  # 10% emporter/livraison
    image_url: Mapped[str | None] = mapped_column(String(500))

    produits: Mapped[list["Produit"]] = relationship(back_populates="categorie", lazy="selectin")
...
class Produit(Base, TimestampMixin):
    ...
    actif: Mapped[bool] = mapped_column(Boolean, default=True)
    ordre: Mapped[int] = mapped_column(Integer, default=0)
    base_sauce: Mapped[str | None] = mapped_column(String(50))  # tomate, creme, bbq

    categorie: Mapped["Categorie"] = relationship(back_populates="produits")
    tailles: Mapped[list["ProduitTaille"]] = relationship(back_populates="produit", lazy="selectin")
    ingredients: Mapped[list["ProduitIngredient"]] = relationship(back_populates="produit", lazy="selectin")
...
class Supplement(Base, TimestampMixin):
    ...
    nom: Mapped[str] = mapped_column(String(200), nullable=False)
    prix: Mapped[float] = mapped_column(Float, nullable=False)
    categorie: Mapped[str | None] = mapped_column(String(100))  # fromage, viande, legume
    actif: Mapped[bool] = mapped_column(Boolean, default=True)
...
class Formule(Base, TimestampMixin):
    ...
    elements: Mapped[list["FormuleElement"]] = relationship(back_populates="formule", lazy="selectin")
...
class FormuleElement(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    formule_id: Mapped[int] = mapped_column(ForeignKey("formules.id"), nullable=False)
    categorie_id: Mapped[int] = mapped_column(ForeignKey("categories.id"), nullable=False)
...
class ProduitIngredient(Base):
    ...
    produit: Mapped["Produit"] = relationship(back_populates="ingredients")
...
class Ingredient(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    nom: Mapped[str] = mapped_column(String(200), nullable=False)
    unite: Mapped[str] = mapped_column(String(50), default="unite")  # kg, litre, unite
