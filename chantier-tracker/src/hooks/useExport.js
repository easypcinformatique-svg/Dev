...
import { LOTS, TOTAL_TTC, TVA_RATE, ECHEANCIER } from '../data/finances.js'
import { ARTISANS } from '../data/artisans.js'
import { daysUntil, formatDateFR } from '../utils/dateUtils.js'
...
export async function exportZip(state) {
  try {
      ...
      for (const [catName, catDocs] of Object.entries(byCategory)) {
        ...
        catDocs.forEach(doc => {
          try {
            ...
            console.warn(`Impossible d'inclure ${doc.name} dans le ZIP:`, err)
          }
        })
      }
    }
    ...
    console.error('Erreur lors de la generation du ZIP:', err)
    throw err
  }
}
