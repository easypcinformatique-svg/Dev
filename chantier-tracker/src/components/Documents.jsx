...
import { formatSize } from '../utils/dateUtils.js'
import { processUploadedFiles, AUTO_CHECK_TIMEOUT } from '../utils/fileUtils.js'
...
export default function Documents({ toggleTask, toggleLegal, completedTasks, completedLegal }) {
  ...
  const handleFiles = useCallback(async (files) => {
    setUploading(true)
    const checked = await processUploadedFiles(files, {
      completedTasks,
      completedLegal,
      toggleTask,
      toggleLegal,
    ...
    if (checked.length > 0) {
      ...
      setTimeout(() => setAutoChecked([]), AUTO_CHECK_TIMEOUT)
    }
            ...
            <div key={doc.id} className="doc-card">
              <div className="doc-header" onClick={() => setPreview(preview?.id === doc.id ? null : doc)}>
                ...
                <div className="doc-info">
                  ...
                  <div className="doc-meta">{formatSize(doc.size)} — {formatDocDate(doc.date)}</div>
                </div>
